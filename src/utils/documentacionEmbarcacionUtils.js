// src/utils/documentacionEmbarcacionUtils.js
// Utilidades para manejo de documentación de embarcación

import { actualizarDocumentacionEmbarcacion, getDocumentacionesEmbarcacion } from "../api/documentacionEmbarcacion";

/**
 * Recalcula y actualiza en base de datos el estado de vencimiento de documentos de embarcación
 ...
 */
export const recalcularDocEmbarcacionVencidos = async (toast = null) => {
  try {
    // Obtener todas las documentaciones desde la API
    const documentaciones = await getDocumentacionesEmbarcacion();
    ...
    if (!documentaciones || documentaciones.length === 0) {
      if (toast?.current) {
        toast.current.show({
          severity: "warn",
          summary: "Sin datos",
          detail: "No hay documentaciones de embarcación para recalcular",
        });
      }
      return [];
    }
    ...
    const updatePromises = documentacionesActualizadas.map(doc => 
      actualizarDocumentacionEmbarcacion(doc.id, {
        embarcacionId: doc.embarcacionId,
    ...
    if (toast?.current) {
      toast.current.show({
        ...
        detail: `Se recalcularon y guardaron ${documentacionesActualizadas.length} documentos de embarcación en la base de datos`,
      });
    }

    return documentacionesActualizadas;
  } catch (error) {
    console.error("Error al recalcular estados de documentación de embarcación:", error);
    if (toast?.current) {
      toast.current.show({
        severity: "error",
        summary: "Error",
