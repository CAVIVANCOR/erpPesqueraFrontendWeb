...
const AsientoContableViewer = ({
  ...
  if (!asiento) {
    return (
      <Message severity="info" text="No hay asiento contable para mostrar" />
    );
  }

  // Separar detalles en DEBE y HABER
  const detallesDebe =
    asiento.detalles?.filter((d) => Number(d.debe) > 0) || [];
  const detallesHaber =
    asiento.detalles?.filter((d) => Number(d.haber) > 0) || [];

  const totalDebe = Number(asiento.totalDebe || 0);
  const totalHaber = Number(asiento.totalHaber || 0);
  const estaCuadrado =
    asiento.estaCuadrado || Math.abs(totalDebe - totalHaber) < 0.01;

  // Obtener símbolo de moneda del asiento
  const simboloMoneda =
    asiento.moneda?.simbolo || asiento.moneda?.codigo || "S/.";
  ...
  const cuentaTemplate = (rowData, tipo) => {
    ...
    return (
      <div>
        ...
        <div
          style={{ fontSize: "0.85rem", color: "#6c757d", marginTop: "4px" }}
        >
          {rowData.glosa}
        </div>
        <div
          ...
          <div className="flex justify-content-between align-items-center">
            <div>
              ...
              <div style={{ fontSize: "0.9rem", color: "#6c757d" }}>
                ...
                {asiento.periodoContable?.nombre || "N/A"}
              </div>
            </div>
            <div className="flex gap-2">
              <Tag
                ...
                severity={
                  asiento.origenAsiento === "AUTOMATICO" ? "success" : "info"
                }
              />
              <Tag
                value={asiento.tipoLibro}
                severity={
                  asiento.tipoLibro === "FISCAL" ? "warning" : "secondary"
                }
              />
              <Tag
