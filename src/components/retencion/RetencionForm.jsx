...
// ════════════════════════════════════════════════════════════
// CONSTANTES DE CONFIGURACIÓN - RETENCIONES
// ════════════════════════════════════════════════════════════

/**
 * ID del tipo "RETENCION" en la tabla TipoProvieneDe
 * Este valor se utiliza para filtrar los estados específicos de retenciones
 * en la tabla EstadoMultiFuncion mediante el campo tipoProvieneDeId
 * 
 * Referencia: TipoProvieneDe.id = 29 → "RETENCION"
 * 
 * Estados disponibles para retenciones (tipoProvieneDeId = 29):
 * - 129: PENDIENTE (secondary) - Retención registrada, pendiente de validación
 * - 130: VALIDADO (success) - Retención validada y confirmada
 * - 131: ASIENTO GENERADO (contrast) - Asiento contable generado automáticamente
 */
const TIPO_PROVIENE_DE_RETENCION = 29;

/**
 * Estado por defecto para nuevas retenciones
 * IMPORTANTE: Este ID corresponde al estado "PENDIENTE" en EstadoMultiFuncion
 * donde tipoProvieneDeId = 29 (RETENCION)
 */
const ESTADO_DEFAULT_PENDIENTE = 129;

/**
 * IDs de estados específicos para retenciones
 */
const ESTADOS_RETENCION = {
  PENDIENTE: 129,           // Retención pendiente
  VALIDADO: 130,            // Retención validada
  ASIENTO_GENERADO: 131,    // Asiento contable generado
};
...
const RetencionForm = forwardRef((props, ref) => {
  const {
    isEdit,
    defaultValues,
    empresas,
  ...
  const [formData, setFormData] = useState({
    ...
    tasaRetencion: defaultValues?.tasaRetencion || 0,
    ...
    importeRetenido: defaultValues?.importeRetenido || MONTO_DEFAULT,
  ...
  // Calcular saldoPendiente cuando cambie importeRetenido o importePagado
  useEffect(() => {
    const nuevoSaldo = Number(formData.importeRetenido) - Number(formData.importePagado || 0);
    if (nuevoSaldo !== formData.saldoPendiente) {
      onChange("saldoPendiente", nuevoSaldo);
    }
  }, [formData.importeRetenido, formData.importePagado]);

  // Calcular estado automáticamente según pagos
  useEffect(() => {
    // Por ahora, el estado se maneja manualmente
    // No hay cálculo automático de estados para retenciones
  }, [
    formData.importeRetenido,
    formData.importePagado,
    formData.saldoPendiente,
    isEdit,
  ]);

  const recargarRetencionDesdeBackend = async () => {
    if (!isEdit || !defaultValues?.id) return;

    try {
      const retencionActualizada = await getRetencionById(defaultValues.id);
      onChange("importeRetenido", retencionActualizada.importeRetenido || 0);
      onChange("importePagado", retencionActualizada.importePagado || 0);
      onChange("saldoPendiente", retencionActualizada.saldoPendiente || 0);
      onChange("estadoPagoId", retencionActualizada.estadoPagoId || ESTADO_DEFAULT_PENDIENTE);
    } catch (error) {
      console.error("Error al recargar retención desde backend:", error);
    }
  };

  useImperativeHandle(ref, () => ({
    recargarRetencionDesdeBackend,
  }));
  ...
  const handleSubmit = () => {
    ...
    const data = {
      ...
      tasaRetencion: Number(formData.tasaRetencion) || 0,
      ...
      importeRetenido: Number(formData.importeRetenido),
        ...
        <TabPanel header="Datos Generales" leftIcon="pi pi-file">
          <div className="p-fluid">
            ...
            >
              ...
              {/* Tasa Retención */}
              <div style={{ flex: 1 }}>
                <label htmlFor="tasaRetencion">
                  Tasa Retención (%)
                </label>
                <InputNumber
                  id="tasaRetencion"
                  value={formData.tasaRetencion}
                  onValueChange={(e) => onChange("tasaRetencion", e.value || 0)}
                  mode="decimal"
                  minFractionDigits={2}
                  maxFractionDigits={2}
                  disabled={readOnly || loading}
            ...
            >
              ...
              {/* Importe Retenido */}
              <div style={{ flex: 1 }}>
                <label htmlFor="importeRetenido">
                  Importe Retenido <span className="text-red-500">*</span>
                </label>
                <InputNumber
                  id="importeRetenido"
                  value={formData.importeRetenido}
                  onValueChange={(e) => onChange("importeRetenido", e.value || 0)}
                  mode="decimal"
                  minFractionDigits={2}
                  maxFractionDigits={2}
                  disabled={readOnly || loading}
            ...
            >
              ...
              <div style={{ flex: 1 }}>
                ...
                <Dropdown
                  ...
                  options={estados
                    ?.filter((e) => Number(e.tipoProvieneDeId) === TIPO_PROVIENE_DE_RETENCION)
                    ?.map((e) => ({
                      label: e.descripcion,
                      value: Number(e.id),
                    })) || []}
          ...
          <TabPanel header="Pagos" leftIcon="pi pi-money-bill">
            <div className="p-fluid">
              <p style={{ marginBottom: "1rem", color: "#666" }}>
                Los pagos de retención se registran en el módulo de Movimientos de Caja
              </p>
