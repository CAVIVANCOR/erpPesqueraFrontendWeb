/**
 * Formulario para gestión de Tipos de Entidad
 *
...
import { crearTipoEntidad, actualizarTipoEntidad } from "../../api/tipoEntidad";
...
const TipoEntidadForm = ({ tipoEntidad, onGuardar, onCancelar }) => {
  const [loading, setLoading] = useState(false);
  const esEdicion = !!tipoEntidad;
  ...
  useEffect(() => {
    if (tipoEntidad) {
      setValue("nombre", tipoEntidad.nombre || "");
      setValue("descripcion", tipoEntidad.descripcion || "");
      setValue("esCliente", tipoEntidad.esCliente || false);
      setValue("esProveedor", tipoEntidad.esProveedor || false);
      setValue("activo", tipoEntidad.activo !== false); // Por defecto true
    } else {
      reset({
        nombre: "",
        descripcion: "",
  ...
  const onSubmit = async (data) => {
    try {
      ...
      if (esEdicion) {
        await actualizarTipoEntidad(tipoEntidad.id, datosNormalizados);
      } else {
        await crearTipoEntidad(datosNormalizados);
      }

      onGuardar();
    } catch (error) {
      console.error("Error al guardar tipo de entidad:", error);
          ...
          <Controller
            ...
            render={({ field }) => (
              <InputText
                id="nombre"
                {...field}
                placeholder="Ingrese el nombre del tipo de entidad"
          ...
          <Controller
            ...
            render={({ field }) => (
              <InputText
                id="descripcion"
                {...field}
                placeholder="Descripción del tipo de entidad (opcional)"
...
export default TipoEntidadForm;
