/**
 * DetalleAccionesPreviasConsumoCard.jsx
 *
 * Componente para mostrar y gestionar las acciones previas de faena de consumo.
 * Sigue el patrón profesional ERP Megui basado en DetalleAccionesPreviasForm.jsx.
 *
 * @author ERP Megui
 * @version 1.0.0
 */
...
import {
  getDetAccionesPreviasFaenaConsumo,
  actualizarDetAccionesPreviasFaenaConsumo,
} from "../../api/detAccionesPreviasFaenaConsumo";
import { useAuthStore } from "../../shared/stores/useAuthStore";
import { getResponsiveFontSize } from "../../utils/utils";
import { abrirPdfEnNuevaPestana } from "../../utils/pdfUtils";
import DetAccionesPreviasFaenaConsumoForm from "../detAccionesPreviasFaenaConsumo/DetAccionesPreviasFaenaConsumoForm";
import { getAllAccionesPreviasFaena } from "../../api/accionesPreviasFaena";

/**
 * Componente DetalleAccionesPreviasConsumoCard
 * @param {Object} props - Props del componente
 * @param {number} props.novedadPescaConsumoId - ID de la novedad de pesca consumo
 * @param {number} props.faenaPescaConsumoId - ID de la faena de pesca consumo
 * @param {Array} props.personal - Array de personal para mostrar nombres completos
 */
const DetalleAccionesPreviasConsumoCard = forwardRef(
  ({ novedadPescaConsumoId, faenaPescaConsumoId, personal = [] }, ref) => {
    ...
    // Cargar acciones previas al montar el componente o cambiar faenaPescaConsumoId
    useEffect(() => {
    ...
    const accionTemplate = (rowData) => {
      const accion = accionesPreviasCatalogo.find(
    ...
    const marcarComoCumplida = async (rowData) => {
      try {
        setLoading(true);
        await actualizarDetAccionesPreviasFaenaConsumo(rowData.id, {
          cumplida: true,
    ...
    const handleEditSubmit = async (formData) => {
      try {
        setEditLoading(true);
        await actualizarDetAccionesPreviasFaenaConsumo(selectedAccion.id, formData);
    ...
    return (
      <div className="card">
        <DataTable
          ...
          emptyMessage="No se encontraron acciones previas para esta faena"
          paginator
          rows={10}
        ...
        >
          ...
          <div className="flex justify-content-center mb-4">
            <Tag
              value={
                accionesPreviasCatalogo.find(
                  (a) => Number(a.id) === Number(selectedAccion?.accionPreviaId)
                )?.nombre || "N/A"
              }
              severity="info"
              style={{
          ...
          {selectedAccion && (
            <DetAccionesPreviasFaenaConsumoForm
              ...
              defaultValues={{
                ...
                faenaPescaConsumoId: selectedAccion.faenaPescaConsumoId,
...
DetalleAccionesPreviasConsumoCard.displayName = "DetalleAccionesPreviasConsumoCard";

export default DetalleAccionesPreviasConsumoCard;