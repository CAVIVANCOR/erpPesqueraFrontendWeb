...
export async function generarYSubirPDFLiquidacionPC(
  ...
  try {
    ...
    const uploadResponse = await fetch(
      `${import.meta.env.VITE_API_URL}/entregas-rendir-consumo/upload-pdf`,
      {
        method: "POST",
        headers: {
          Authorization: `Bearer ${token}`,
    ...
    const updateResponse = await fetch(
      `${import.meta.env.VITE_API_URL}/entregas-rendir-consumo/${entregaARendir.id}`,
      {
        ...
        body: JSON.stringify({
          novedadPescaConsumoId: entregaARendir.novedadPescaConsumoId,
    ...
    if (!updateResponse.ok) {
      throw new Error(
        "Error al actualizar la entrega a rendir con la URL del PDF",
      );
    }
...
async function generarPDFLiquidacionPC(entregaARendir, movimientos, empresa) {
  ...
  yPosition = await dibujaEncabezadoPDFLiquidacionPC(
    page,
    pdfDoc,
    entregaARendir,
    empresa,
