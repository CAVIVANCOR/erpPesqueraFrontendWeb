// src/components/liquidacionFaenaConsumo/LiquidacionFaenaConsumoForm.jsx
// Formulario profesional para LiquidacionFaenaConsumo. Cumple regla transversal ERP Megui:
...
import { createLiquidacionFaenaConsumo, updateLiquidacionFaenaConsumo } from '../../api/liquidacionFaenaConsumo';
import { getAllFaenaPescaConsumo } from '../../api/faenaPescaConsumo';
import { getEmpresas } from '../../api/empresa';
import { getPersonal } from '../../api/personal';

/**
 * Componente LiquidacionFaenaConsumoForm
 * Formulario profesional para gestión de liquidaciones de faenas de consumo
 * Organizado en pestañas para mejor UX en formularios complejos
 */
const LiquidacionFaenaConsumoForm = ({ liquidacion, onSave, onCancel }) => {
  ...
  useEffect(() => {
    if (liquidacion) {
      reset({
        faenaPescaConsumoId: liquidacion.faenaPescaConsumoId,
  ...
  const cargarDatosIniciales = async () => {
    try {
      setLoading(true);
      const [faenasData, empresasData, personalData] = await Promise.all([
        getAllFaenaPescaConsumo(),
        getEmpresas(),
        getPersonal()
      ]);
      
      // Normalizar IDs a numéricos según regla profesional ERP Megui
      setFaenas(faenasData.map(f => ({ ...f, id: Number(f.id) })));
  ...
  const onSubmit = async (data) => {
    try {
      ...
      if (liquidacion) {
        ...
        await createLiquidacionFaenaConsumo(payload);
        toast.current?.show({
          severity: 'success',
          summary: 'Éxito',
  ...
  return (
    <div className="liquidacion-faena-consumo-form">
      <Toast ref={toast} />
                ...
                <Controller
                  ...
                  render={({ field }) => (
                    <Dropdown
                      ...
                      options={faenas}
                      optionLabel="numeroFaena"
                      optionValue="id"
                      placeholder="Seleccionar faena"
                      className={`w-full ${errors.faenaPescaConsumoId ? 'p-invalid' : ''}`}
                      filter
                      showClear
...
export default LiquidacionFaenaConsumoForm;
