...
import { recalcularCascadaDesdeFaena } from "../../api/recalcularToneladas";

export default function CalaForm({
  isEdit,
  defaultValues,
  onSubmit,
  onCancel,
  ...
  const [faenaPescaId, setFaenaPescaId] = React.useState(
    defaultValues.faenaPescaId || ""
  );
  const [temporadaPescaId, setTemporadaPescaId] = React.useState(
    defaultValues.temporadaPescaId || ""
  );
  const [fechaHoraInicio, setFechaHoraInicio] = React.useState(
    defaultValues.fechaHoraInicio
      ? new Date(defaultValues.fechaHoraInicio)
      : new Date()
  );
  const [fechaHoraFin, setFechaHoraFin] = React.useState(
    defaultValues.fechaHoraFin
      ? new Date(defaultValues.fechaHoraFin)
      : new Date()
  );
  ...
  const [profundidadM, setProfundidadM] = React.useState(
    defaultValues.profundidadM || 0
  );
  const [toneladasCapturadas, setToneladasCapturadas] = React.useState(
    defaultValues.toneladasCapturadas || 0
  );
  ...
  const handleSubmit = async (e) => {
    ...
    // RECÁLCULO AUTOMÁTICO: Cuando se modifica Cala → recalcular en cascada desde Faena
    if (faenaPescaId) {
      try {
        await recalcularCascadaDesdeFaena(faenaPescaId);
      } catch (error) {
    ...
    <form onSubmit={handleSubmit} className="p-fluid">
      ...
      <div className="p-field">
        ...
        <InputText
          id="faenaPescaId"
          value={faenaPescaId}
          onChange={(e) => setFaenaPescaId(e.target.value)}
          required
          disabled={loading}
        />
      </div>
      ...
      <div className="p-field">
        ...
        <div style={{ border: '2px solid red', padding: '10px', margin: '10px 0' }}>
          <p>DEBUG: Componente CoordinatesInput debería aparecer aquí</p>
          <div style={{ 
            ...
            >
              Test INLINE: Establecer Coordenadas
            </button>
          </div>
        </div>
      </div>
