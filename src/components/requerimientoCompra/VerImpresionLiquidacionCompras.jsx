...
const VerImpresionLiquidacionCompras = ({
  ...
  const generarPdfWrapper = async () => {
    ...
    const response = await fetch(
      `${import.meta.env.VITE_API_URL}/entregas-rendir-compras/${datosEntrega.id}`,
      { headers }
    );
    ...
    const resultado = await generarYSubirPDFLiquidacionCompras(
      entregaCompleta,
      movimientos,
      empresa
    );
  ...
  return (
    <Card title="Liquidación PDF - Compras" className="mb-4" style={{ fontSize: "12px" }}>
      <PDFGeneratedUploader
        ...
        moduleName="entregas-rendir-compras"
        entityId={entregaARendirId}
        fileName={`liquidacion-compras-${entregaARendirId}.pdf`}
