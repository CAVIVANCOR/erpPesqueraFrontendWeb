/**
 * LiquidacionComprasPDF.js
 * Generador de PDF para liquidación de entregas a rendir de Compras
 * @author ERP Megui
 * @version 1.0.0
 */
...
import { dibujaEncabezadoPDFLiquidacionCompras } from "./dibujaEncabezadoPDFLiquidacionCompras";
import { dibujaTotalesYFirmaPDFLiquidacionCompras } from "./dibujaTotalesYFirmaPDFLiquidacionCompras";

export async function generarYSubirPDFLiquidacionCompras(
  entregaARendir,
  movimientos,
  empresa
) {
  try {
    const pdfBytes = await generarPDFLiquidacionCompras(
      entregaARendir,
      movimientos,
      empresa
    );
    ...
    const nombreArchivo = `liquidacion_compras_${entregaARendir.id}_${Date.now()}.pdf`;
    ...
    console.error("Error en generarYSubirPDFLiquidacionCompras:", error);
    return { success: false, error: error.message };
  }
}

async function generarPDFLiquidacionCompras(entregaARendir, movimientos, empresa) {
  ...
  yPosition = dibujaEncabezadoPDFLiquidacionCompras(
  ...
  yPosition = dibujaTotalesYFirmaPDFLiquidacionCompras(
...
function dibujarTablaMovimientos(page, movimientos, fontBold, fontRegular, startY, pageWidth) {
  ...
  page.drawRectangle({
    x: margin, y: yPosition - 20, width: tableWidth, height: 20,
  ...
  movimientos.forEach((mov, i) => {
    if (i % 2 === 0) {
      page.drawRectangle({
        x: margin, y: yPosition - 15, width: tableWidth, height: 15,
    ...
    if (esIngreso) {
      page.drawText(monto, {
        x: margin + 350, y: yPosition - 10, size: 8, font: fontRegular,
        color: rgb(0, 0.5, 0),
      });
    } else {
      page.drawText(monto, {
        x: margin + 425, y: yPosition - 10, size: 8, font: fontRegular,
        color: rgb(0.7, 0, 0),
      });
    }
...
export default generarYSubirPDFLiquidacionCompras;
