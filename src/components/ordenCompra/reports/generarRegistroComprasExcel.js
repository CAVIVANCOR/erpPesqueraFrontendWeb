...
export async function generarRegistroComprasExcel(data) {
  ...
  ordenesCompra.forEach(oc => {
    ...
    const baseGravada = subtotalGravadoPEN;
    const exonerado = subtotalExoneradoPEN;
    const inafecto = subtotalInafectoPEN;
    ...
    worksheet.getCell(currentRow, 19).value = exonerado; // Exonerado
    worksheet.getCell(currentRow, 19).numFmt = '#,##0.00';
    worksheet.getCell(currentRow, 20).value = inafecto; // Inafecto
