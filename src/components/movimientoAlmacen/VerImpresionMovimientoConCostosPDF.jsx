  ...
  return (
    <Card>
      <PDFGeneratedUploader
        ...
        fileName={`movimiento-almacen-costos-${datosMovimiento.numeroDocumento || movimientoId}.pdf`}
        buttonLabel="PDF con Costos"
        ...
        onGenerateComplete={(url) => {
          setPdfUrl(url);
          if (datosMovimiento) {
            datosMovimiento.urlMovAlmacenConCostosPdf = url;
          }
          if (onPdfGenerated) onPdfGenerated(url);
        }}
        initialPdfUrl={datosMovimiento?.urlMovAlmacenConCostosPdf}
      />
    </Card>
  );
};
