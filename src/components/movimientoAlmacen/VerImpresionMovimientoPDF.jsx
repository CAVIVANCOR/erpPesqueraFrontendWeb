/**
 * VerImpresionMovimientoPDF.jsx
 *
 * Card para generar y visualizar el PDF del movimiento de almacén SIN COSTOS.
 * Permite generar, visualizar y descargar PDFs de movimientos.
...
/**
 * Componente VerImpresionMovimientoPDF
...
const VerImpresionMovimientoPDF = ({
    ...
    try {
      ...
      if (movimientoEnriquecido.conceptoMovAlmacen) {
        ...
        if (movimientoEnriquecido.conceptoMovAlmacen.almacenOrigenId && 
          ...
          // Intentar obtener del almacenOrigenInfo si está disponible
      ...
      if (resultado.success) {
        ...
        if (datosMovimiento && urlPdf) {
          datosMovimiento.urlMovAlmacenPdf = urlPdf;
        }
        ...
        toast.current.show({
          severity: "success",
          summary: "Éxito",
          detail: "PDF sin costos generado y guardado correctamente",
          life: 3000,
        });
      } else {
  ...
  const handleDescargarPDF = () => {
    if (pdfUrl) {
      const nombreArchivo = `movimiento-almacen-${
      ...
      <div className="p-fluid">
        ...
        {!pdfUrl && (
          <div style={{ marginTop: "1rem" }}>
            <Message
              severity="warn"
              text='No hay PDF generado. Use el botón "PDF sin Costos" para generar el documento del movimiento de almacén.'
              style={{ width: "100%" }}
            />
          </div>
        )}
