...
export function dibujaEncabezadoPDFLiquidacionMovAlmacen(page, pdfDoc, entregaARendir, empresa, fontBold, fontRegular, startY, pageWidth) {
  ...
  yPosition -= 10;
  page.drawText("MOVIMIENTO DE ALMACÉN", {
    x: pageWidth / 2 - 80, y: yPosition, size: 12, font: fontBold, color: rgb(0, 0, 0.5),
  });
  yPosition -= 30;
  ...
  const movimientoAlmacen = entregaARendir.movimientoAlmacen
    ? `MOV-${entregaARendir.movimientoAlmacen.id} - ${entregaARendir.movimientoAlmacen.numeroDocumento || ""}`
    : "N/A";
  page.drawText(`Movimiento Almacén: ${movimientoAlmacen}`, { x: margin + 10, y: yPosition, size: 9, font: fontBold });
  yPosition -= 15;
