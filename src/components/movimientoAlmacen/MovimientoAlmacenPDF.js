...
export async function generarPDFMovimientoAlmacen(
  ...
  try {
    ...
    // Preparar datos del movimiento (se usará en la función de encabezado)
    ...
    if (movimiento.numGuiaSunat) {
      ...
      if (movimiento.fechaGuiaSunat) {
        datosMovimiento.push(["Fecha Guía SUNAT:", formatearFecha(movimiento.fechaGuiaSunat)]);
      }
    }
    ...
    // Función para dibujar encabezado completo del documento
    const dibujarEncabezadoDocumento = async (pag, incluirDatos = true) => {
      let yPos = height - 50;
    ...
    yPosition = await dibujarEncabezadoDocumento(page, true);
    ...
    const tituloTabla = incluirCostos ? "DETALLES DEL MOVIMIENTO (CON COSTOS)" : "DETALLES DEL MOVIMIENTO";
    ...
    for (let index = 0; index < detalles.length; index++) {
      ...
      rowData.forEach((data, i) => {
        ...
        let xTexto = xPos + 2;
        if (i === 2) { // Cantidad - siempre alineada a la derecha
          xTexto = xPos + colWidths[i] - textoWidth - 2;
        } else if (!incluirCostos && i === 4) { // Peso - solo cuando no incluye costos
          xTexto = xPos + colWidths[i] - textoWidth - 2;
        } else if (incluirCostos && (i === 6 || i === 7)) { // Costo Unitario y Costo Total - solo cuando incluye costos
          xTexto = xPos + colWidths[i] - textoWidth - 2;
        }
    ...
    if (incluirCostos) {
      ...
      page.drawText("TOTAL:", {
        x: margin + colWidths[0] + colWidths[1] + colWidths[2] + colWidths[3] + colWidths[4] + colWidths[5] + 5,
        y: yPosition,
        size: 8,
        font: fontBold,
      });
      ...
      const totalX = margin + colWidths[0] + colWidths[1] + colWidths[2] + colWidths[3] + colWidths[4] + colWidths[5] + colWidths[6];
    ...
    // SECCIÓN DE FIRMAS - Posición fija en la última página
    const firmaYPosition = 90; // Posición fija para las firmas
    ...
    // FIRMA DERECHA - Entidad Comercial (usar la misma yPosition inicial)
