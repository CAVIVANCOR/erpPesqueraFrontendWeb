...
export function dibujaEncabezadoPDFLiquidacionVentas(page, entregaARendir, empresa, fontBold, fontRegular, startY, pageWidth) {
  ...
  page.drawText("VENTAS", {
    x: pageWidth / 2 - 30, y: yPosition, size: 12, font: fontBold, color: rgb(0, 0, 0.5),
  });
  yPosition -= 30;
  ...
  const cotizacion = entregaARendir.cotizacionVentas
    ? `COT-${entregaARendir.cotizacionVentas.id}`
    : "N/A";
  page.drawText(`Cotización: ${cotizacion}`, { x: margin + 10, y: yPosition, size: 9, font: fontBold });
  yPosition -= 15;
