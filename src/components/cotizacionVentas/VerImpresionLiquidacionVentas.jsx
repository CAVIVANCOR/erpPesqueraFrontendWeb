...
const VerImpresionLiquidacionVentas = ({
  ...
  const handleGenerarPDF = async () => {
    ...
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_URL}/entregas-rendir-ventas/${datosEntrega.id}`, { headers });
      ...
      const resultado = await generarYSubirPDFLiquidacionVentas(entregaCompleta, movimientos, empresa);
      ...
      <div style={{ display: "flex", gap: 10, marginBottom: 20, flexWrap: "wrap" }}>
        ...
        {pdfUrl && (
          <>
            ...
            <Button label="Descargar" icon="pi pi-download" onClick={() => descargarPdf(pdfUrl, `liquidacion_ventas_${entregaARendirId}.pdf`)} severity="help" />
          </>
        )}
      </div>
      {pdfUrl ? (
