/**
 * Formulario para gestión de Tipos de Embarcación
 *
...
import { crearTipoEmbarcacion, actualizarTipoEmbarcacion } from "../../api/tipoEmbarcacion";
...
const TipoEmbarcacionForm = ({ tipoEmbarcacion, onGuardar, onCancelar }) => {
  const [loading, setLoading] = useState(false);
  const esEdicion = !!tipoEmbarcacion;
  ...
  useEffect(() => {
    if (tipoEmbarcacion) {
      setValue("codigo", tipoEmbarcacion.codigo || "");
      setValue("nombre", tipoEmbarcacion.nombre || "");
      setValue("descripcion", tipoEmbarcacion.descripcion || "");
      setValue("cesado", tipoEmbarcacion.cesado || false);
    } else {
      reset({
        codigo: "",
        nombre: "",
  ...
  }, [tipoEmbarcacion, setValue, reset]);
  ...
  const onSubmit = async (data) => {
    try {
      ...
      if (esEdicion) {
        await actualizarTipoEmbarcacion(tipoEmbarcacion.id, datosNormalizados);
      } else {
        await crearTipoEmbarcacion(datosNormalizados);
      }

      onGuardar();
    } catch (error) {
      console.error("Error al guardar tipo de embarcación:", error);
          ...
          <Controller
            ...
            render={({ field }) => (
              <InputText
                id="descripcion"
                {...field}
                placeholder="Descripción del tipo de embarcación (opcional)"
...
export default TipoEmbarcacionForm;
