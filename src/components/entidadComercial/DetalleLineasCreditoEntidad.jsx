...
const esquemaValidacionLineaCredito = yup.object().shape({
  montoMaximo: yup.number().required("El monto máximo es requerido").min(0, "El monto debe ser mayor a 0"),
  monedaId: yup.number().required("La moneda es requerida"),
  diasCredito: yup.number().required("Los días de crédito son requeridos").min(1, "Debe ser mayor a 0"),
  vigenteDesde: yup.date().required("La fecha de inicio es requerida"),
  vigenteHasta: yup.date().nullable().test(
    "fecha-hasta-mayor",
...
const DetalleLineasCreditoEntidad = forwardRef(({
  ...
  const abrirDialogoEdicion = (lineaCredito) => {
    ...
    setValue("vigenteHasta", lineaCredito.vigenteHasta ? new Date(lineaCredito.vigenteHasta) : null);
  ...
  const eliminar = async () => {
    try {
      ...
      const nuevasLineas = lineasCreditoData.filter((l) => l.id !== lineaCreditoAEliminar.id);
      ...
      toast.current?.show({
        severity: "error",
        summary: "Error",
        detail: error.response?.data?.message || "Error al eliminar la línea de crédito",
        life: 3000,
      });
    } finally {
      setLoading(false);
  ...
  const onSubmitLineaCredito = async (data) => {
    try {
      ...
      let resultado;
      const esEdicion = lineaCreditoSeleccionada && lineaCreditoSeleccionada.id;

      if (esEdicion) {
        // Actualizar línea de crédito existente
        resultado = await actualizarLineaCreditoEntidad(lineaCreditoSeleccionada.id, datosNormalizados);
      ...
      toast.current?.show({
        severity: "error",
        summary: "Error",
        detail: error.response?.data?.message || "Error al guardar la línea de crédito",
        life: 3000,
      });
    } finally {
      setLoading(false);
  ...
  const monedaTemplate = (rowData) => {
    const moneda = monedasData.find(m => Number(m.value) === Number(rowData.monedaId));
  ...
  return (
    <div className="p-4">
      ...
      >
        <Column field="montoMaximo" header="Monto Máximo" body={montoTemplate} sortable />
        <Column field="monedaId" header="Moneda" body={monedaTemplate} sortable />
        <Column field="diasCredito" header="Días Crédito" sortable />
        <Column field="vigenteDesde" header="Vigente Desde" body={(rowData) => fechaTemplate(rowData, 'vigenteDesde')} sortable />
        <Column field="vigenteHasta" header="Vigente Hasta" body={(rowData) => fechaTemplate(rowData, 'vigenteHasta')} sortable />
      ...
      <Dialog
        header={lineaCreditoSeleccionada ? "Editar Línea de Crédito" : "Nueva Línea de Crédito"}
        visible={dialogVisible}
        onHide={cerrarDialogo}
        style={{ width: "800px" }}
        modal
      >
        <form onSubmit={handleSubmit(onSubmitLineaCredito)} className="p-fluid">
