...
// Formulario profesional para DetalleDocEmbarcacion - ERP Megui
...
/**
 * Formulario DetalleDocEmbarcacionForm
 *
 * Formulario profesional para gestión de documentos de embarcación.
 * Características:
 ...
 */
export default function DetalleDocEmbarcacionForm({
  detalle,
  documentosPesca = [],
  onGuardadoExitoso,
  onCancelar,
  ...
  const onSubmit = async (data) => {
    try {
      ...
      if (detalle?.id) {
        await actualizarDetalleDocEmbarcacion(detalle.id, payload);
      } else {
        await crearDetalleDocEmbarcacion(payload);
      }
  ...
  const handleDescargarPDF = () => {
    if (urlDocEmbarcacion) {
      descargarPdf(
        urlDocEmbarcacion,
        toast,
        `documento-embarcacion-${detalle?.documentoPescaId || "sin-id"}-${
