/**
 * EntregaARendirContratoCard.jsx
 *
 * Card para gestionar la entrega a rendir única por contrato de servicio.
 * Muestra el registro único de EntregaARendirContratoServicios y permite gestionar sus movimientos detallados.
 * Se habilita solo cuando existe un contrato guardado con responsableId válido.
 *
 * @author ERP Megui
 * @version 1.0.0
 */
...
import DetEntregaRendirContrato from "./DetEntregaRendirContrato";
import { formatearNumero } from "../../utils/utils";
import {
  getAllEntregaARendirContratoServicios,
  crearEntregaARendirContratoServicios,
  actualizarEntregaARendirContratoServicios,
  eliminarEntregaARendirContratoServicios,
} from "../../api/entregaARendirContratoServicios";
import { getAllDetMovsEntregaRendirContratoServicios } from "../../api/detMovsEntregaRendirContratoServicios";
import { getProductos } from "../../api/producto";
import { useAuthStore } from "../../shared/stores/useAuthStore";

export default function EntregaARendirContratoCard({
  contratoServicio,
  personal = [],
  centrosCosto = [],
  tiposMovimiento = [],
  ...
  // Estados para EntregaARendirContratoServicios (única)
  ...
  // Estados para DetMovsEntregaRendirContratoServicios
  ...
  // Verificar y cargar entrega a rendir cuando cambie el contrato
  useEffect(() => {
    if (contratoServicio?.id) {
  ...
  // Cargar productos cuando cambie el contrato
  useEffect(() => {
    if (contratoServicio?.empresaId) {
      cargarProductos();
    }
  }, [contratoServicio?.empresaId]);
  ...
  }, [contratoServicio?.empresaId, entidadesComerciales]);
  ...
  /**
   * Verificar si existe una entrega a rendir para este contrato
   */
  ...
  const crearEntregaAutomatica = async () => {
    ...
    try {
      const dataToCreate = {
        contratoServicioId: Number(contratoServicio.id),
        respEntregaRendirId: Number(contratoServicio.responsableId),
  ...
  const handleGuardarCambios = async () => {
    ...
    try {
      const dataToUpdate = {
        contratoServicioId: Number(entregaARendir.contratoServicioId),
  ...
  if (!contratoServicio?.id) {
    return (
      <Panel className="mt-3">
        <Message
          severity="info"
          text="Guarde el contrato primero para poder gestionar entregas a rendir"
        />
      </Panel>
    );
  }
  ...
  if (!entregaARendir) {
    return (
      <Panel className="mt-3">
        <Message
          severity="info"
          text="No existe una entrega a rendir para este contrato de servicio"
        />
        ...
        header={
          <div className="flex justify-content-between align-items-center w-full">
            <span>
              ...
              Entrega a Rendir - Contrato #{contratoServicio.numeroCompleto}
            </span>
          </div>
        }
        className="mt-3"
