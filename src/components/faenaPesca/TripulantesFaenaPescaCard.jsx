/**
 * TripulantesFaenaPescaCard.jsx
 *
 * Componente para mostrar y gestionar los tripulantes de una faena de pesca.
 * Permite listar y ver detalles de tripulantes (solo lectura).
 * Los tripulantes se crean automáticamente al iniciar la temporada.
 *
 * @author ERP Megui
 * @version 1.0.0
 */
...
const TripulantesFaenaPescaCard = ({
  ...
  // Cargar tripulantes al montar el componente o cambiar faenaPescaId
  useEffect(() => {
    if (faenaPescaId) {
      cargarTripulantes();
    }
  ...
  const cargarTripulantes = async () => {
    ...
    try {
      const data = await getTripulantesPorFaena(faenaPescaId);
      setTripulantes(data || []);
  ...
  const handleGuardarTripulante = async (datosActualizados) => {
    try {
      await actualizarTripulanteFaena(editingTripulante.id, datosActualizados);
...
export default TripulantesFaenaPescaCard;