...
export default function CuentaPorCobrarForm({
  ...
  const [clientesFiltrados, setClientesFiltrados] = useState([]);
    useEffect(() => {
    if (clientes && clientes.length > 0 && empresaId) {
      const clientesPorEmpresa = clientes.filter(
        (c) => Number(c.empresaId) === Number(empresaId)
      );
      setClientesFiltrados(clientesPorEmpresa);
    } else {
        ...
        >
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <Checkbox
              inputId="esContado"
              checked={esContado}
              onChange={(e) => setEsContado(e.checked)}
              disabled={!puedeEditar}
            />
            <label
              htmlFor="esContado"
