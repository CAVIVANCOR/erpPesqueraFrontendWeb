// src/components/tesoreria/PagarDeudaTributariaDialog.jsx
...
const PagarDeudaTributariaDialog = ({
  deuda,
  cuentasCorrientes = [],
  mediosPago = [],
  onSubmit,
  ...
  const validarFormulario = () => {
    ...
    if (cuentaSeleccionada && deuda) {
      ...
      const itf = Number(formData.itf || 0);
      const comision = Number(formData.comision || 0);
      const totalAPagar = montoPago + itf + comision;

      if (saldoCuenta < totalAPagar) {
        newErrors.cuentaCorrienteOrigenId = `Saldo insuficiente. Disponible: ${deuda.moneda?.simbolo} ${formatearNumero(saldoCuenta)}. Total a descontar: ${deuda.moneda?.simbolo} ${formatearNumero(totalAPagar)}`;
      }
    }
  ...
  const handleSubmit = async (e) => {
    ...
    toast?.current?.show({
      severity: "info",
      summary: "Procesando",
      detail: "Procesando pago de deuda tributaria...",
      life: 2000,
    });
  ...
  return (
    <form onSubmit={handleSubmit} className="p-fluid">
      ...
      <Panel header="📋 Información de la Deuda Tributaria" className="mb-3">
        <div className="grid">
          {/* Fila 1 */}
          ...
          <div className="col-12 md:col-4">
            <label className="block mb-2 font-bold">Entidad Recaudadora</label>
            <InputText
              value={deuda?.tipoDeuda?.entidadRecaudadora?.nombre || "N/A"}
              disabled
              style={{
                fontWeight: "bold",
                backgroundColor: "#f8f9fa",
          ...
          <div className="col-12 md:col-3">
            <label className="block mb-2 font-bold">Periodo</label>
            <InputText
              value={deuda?.periodo || "N/A"}
              disabled
              style={{
                textAlign: "center",
                fontWeight: "bold",
      ...
      <Panel header="💰 Datos del Pago" className="mb-3">
        <div className="grid">
          ...
          {/* Fila 2: ITF y Comisión */}
          <div className="col-12 md:col-6">
            <label htmlFor="itf" className="block mb-2 font-bold">
              ITF (Impuesto a las Transacciones Financieras)
            </label>
            <InputNumber
              id="itf"
              value={formData.itf}
              onValueChange={(e) => handleChange("itf", e.value)}
              mode="decimal"
              minFractionDigits={2}
              maxFractionDigits={2}
              prefix={`${deuda?.moneda?.simbolo || ""} `}
            ...
            <small className="text-muted block mt-1">
              Opcional: Impuesto que cobra el banco por la transacción
            </small>
          </div>

          <div className="col-12 md:col-6">
            <label htmlFor="comision" className="block mb-2 font-bold">
              Comisión Bancaria
            </label>
            <InputNumber
              id="comision"
              value={formData.comision}
              onValueChange={(e) => handleChange("comision", e.value)}
              mode="decimal"
              minFractionDigits={2}
              maxFractionDigits={2}
              prefix={`${deuda?.moneda?.simbolo || ""} `}
            ...
            />
            <small className="text-muted block mt-1">
              Opcional: Comisión que cobra el banco por la operación
            </small>
          </div>

          {/* Mostrar total a descontar */}
            ...
            >
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <span style={{ fontWeight: "bold", fontSize: "1.1rem" }}>
                  💰 Total a Descontar de la Cuenta:
                </span>
                <span style={{ fontWeight: "bold", fontSize: "1.2rem", color: "#1976d2" }}>
                  {deuda?.moneda?.simbolo || ""} {formatearNumero(
                    Number(formData.montoPago || 0) + 
                    Number(formData.itf || 0) + 
                    Number(formData.comision || 0)
                  )}
                </span>
              </div>
              <small style={{ color: "#555", display: "block", marginTop: "0.5rem" }}>
                Monto Pago: {deuda?.moneda?.simbolo} {formatearNumero(formData.montoPago || 0)} + 
                ITF: {deuda?.moneda?.simbolo} {formatearNumero(formData.itf || 0)} + 
                Comisión: {deuda?.moneda?.simbolo} {formatearNumero(formData.comision || 0)}
              </small>
            </div>
          </div>
          ...
          <div className="col-12 md:col-6">
            <label htmlFor="numeroConstancia" className="block mb-2 font-bold">
              Número de Constancia
            </label>
            <InputText
              id="numeroConstancia"
              value={formData.numeroConstancia}
              onChange={(e) => handleChange("numeroConstancia", e.target.value)}
              placeholder="Ej: Constancia SUNAT 123456"
            />
            <small className="text-muted block mt-1">
              Opcional: Número de constancia de pago de SUNAT u otra entidad
            </small>
          </div>
...
export default PagarDeudaTributariaDialog;