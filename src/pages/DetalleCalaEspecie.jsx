// src/pages/DetalleCalaEspecie.jsx
// Pantalla CRUD profesional para DetalleCalaEspecie. Cumple la regla transversal ERP Megui.
...
import DetalleCalaEspecieForm from "../components/detalleCalaEspecie/DetalleCalaEspecieForm";
import { getAllDetalleCalaEspecie, createDetalleCalaEspecie, updateDetalleCalaEspecie, deleteDetalleCalaEspecie } from "../api/detalleCalaEspecie";
import { useAuthStore } from "../shared/stores/useAuthStore";

export default function DetalleCalaEspecie() {
  ...
  const cargarItems = async () => {
    setLoading(true);
    try {
      const data = await getAllDetalleCalaEspecie();
      setItems(data);
    } catch (err) {
  ...
  const handleDeleteConfirm = async () => {
    ...
    setLoading(true);
    try {
      await deleteDetalleCalaEspecie(toDelete.id);
  ...
  return (
    <div className="p-fluid">
      ...
      <div className="p-d-flex p-jc-between p-ai-center" style={{ marginBottom: 16 }}>
        <h2>Gestión de Detalle Cala Especie</h2>
      ...
      <DataTable value={items} loading={loading} dataKey="id" paginator rows={10} onRowClick={e => handleEdit(e.data)} style={{ cursor: "pointer" }}>
        ...
        <Column field="calaId" header="Cala" />
        <Column field="especieId" header="Especie" />
      ...
      <Dialog header={editing ? "Editar Detalle" : "Nuevo Detalle"} visible={showDialog} style={{ width: 600 }} onHide={() => setShowDialog(false)} modal>
        <DetalleCalaEspecieForm
          isEdit={!!editing}
