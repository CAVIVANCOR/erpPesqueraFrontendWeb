...
import { getMovimientosAlmacen, crearMovimientoAlmacen, actualizarMovimientoAlmacen, eliminarMovimientoAlmacen } from "../api/movimientoAlmacen";
...
export default function MovimientoAlmacen() {
  ...
  const cargarDatos = async () => {
    setLoading(true);
    try {
      const [movimientosData, empresasData, tiposData, entidadesData] = await Promise.all([
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      toast.current.show({ severity: "success", summary: "Eliminado", detail: "Movimiento de almacén eliminado correctamente." });
      cargarDatos();
    } catch (err) {
  ...
  const handleFormSubmit = async (data) => {
    ...
    try {
      if (editing && editing.id) {
        await actualizarMovimientoAlmacen(editing.id, data);
        toast.current.show({ severity: "success", summary: "Actualizado", detail: "Movimiento de almacén actualizado." });
      } else {
        await crearMovimientoAlmacen(data);
        toast.current.show({ severity: "success", summary: "Creado", detail: "Movimiento de almacén creado." });
      }
      setShowDialog(false);
      setEditing(null);
      cargarDatos();
  ...
  return (
    <div className="p-fluid">
      <Toast ref={toast} />
      <ConfirmDialog visible={showConfirm} onHide={() => setShowConfirm(false)} message="¿Está seguro que desea eliminar este movimiento de almacén?" header="Confirmar eliminación" icon="pi pi-exclamation-triangle" acceptClassName="p-button-danger" accept={handleDeleteConfirm} reject={() => setShowConfirm(false)} />
      ...
      <DataTable value={items} loading={loading} dataKey="id" paginator rows={10} onRowClick={e => handleEdit(e.data)} style={{ cursor: "pointer" }}>
        ...
        <Column field="tipoDocumentoId" header="Tipo Doc." body={tipoDocumentoNombre} />
        <Column field="fechaDocumento" header="Fecha" body={rowData => fechaTemplate(rowData, 'fechaDocumento')} />
      ...
      <Dialog header={editing ? "Editar Movimiento de Almacén" : "Nuevo Movimiento de Almacén"} visible={showDialog} style={{ width: 1000 }} onHide={() => setShowDialog(false)} modal>
