// src/pages/contabilidad/Retencion.jsx
...
import RetencionForm from "../../components/contabilidad/retencion/RetencionForm";
import {
  getRetenciones,
  getRetencionById,
  deleteRetencion,
} from "../../api/contabilidad/retencion";
...
export default function Retencion({ ruta }) {
  ...
  const cargarDatos = async () => {
    setLoading(true);
    try {
      const [retencionesData, empresasData, estadosData, monedasData, proveedoresData] =
        await Promise.all([
          getRetenciones(),
          getEmpresas(),
          getEstadosMultiFuncionPorTipoProviene("RETENCION"),
          getMonedas(),
          getEntidadesComerciales(),
        ]);
      setItems(retencionesData);
  ...
  const handleEditar = async (row) => {
    try {
      const data = await getRetencionById(row.id);
      setSelected(data);
      setIsEdit(true);
      setShowDialog(true);
    } catch (error) {
  ...
  const confirmarEliminar = async () => {
    try {
      await deleteRetencion(confirmState.row.id);
      toast.current?.show({
        severity: "success",
        summary: "Éxito",
      ...
      toast.current?.show({
        severity: "error",
        summary: "Error",
        detail: error.message || "Error al eliminar retención",
        life: 3000,
      });
    } finally {
  ...
  const handleGuardar = () => {
    ...
    toast.current?.show({
      severity: "success",
      summary: "Éxito",
      detail: `Retención ${isEdit ? "actualizada" : "creada"} correctamente`,
      life: 3000,
    });
  };
  ...
  const montoBodyTemplate = (rowData) => {
    return `${rowData.moneda?.simbolo || ""} ${Number(rowData.importeRetenido || 0).toFixed(2)}`;
  };

  const accionesBodyTemplate = (rowData) => {
    const estadoId = Number(rowData.estadoId);
    const esPendiente = estadoId === 129;
  ...
  return (
    <div className="card">
      ...
      >
        ...
        <Column field="tipoRetencion.nombre" header="Tipo" sortable />
        <Column
