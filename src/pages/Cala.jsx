// src/pages/Cala.jsx
// Pantalla CRUD profesional para Cala. Cumple la regla transversal ERP Megui.
...
export default function Cala() {
  ...
  const handleDeleteConfirm = async () => {
    ...
    setLoading(true);
    try {
      await eliminarCala(toDelete.id);
  ...
  return (
    <div className="p-fluid">
      ...
      <DataTable value={items} loading={loading} dataKey="id" paginator rows={10} onRowClick={e => handleEdit(e.data)} style={{ cursor: "pointer" }}>
        ...
        <Column field="temporadaPescaId" header="Temporada" />
        <Column field="fechaHoraInicio" header="Inicio" body={rowData => new Date(rowData.fechaHoraInicio).toLocaleString()} />
      ...
      <Dialog header={editing ? "Editar Cala" : "Nueva Cala"} visible={showDialog} style={{ width: 800 }} onHide={() => setShowDialog(false)} modal>
        <CalaForm
          isEdit={!!editing}
