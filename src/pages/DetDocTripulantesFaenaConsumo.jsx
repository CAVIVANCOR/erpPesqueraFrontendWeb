...
import {
  getAllDetDocTripulantesFaenaConsumo,
  deleteDetDocTripulantesFaenaConsumo,
 ...
 */
const DetDocTripulantesFaenaConsumo = ({ personal = [] }) => { // ← AGREGAR prop
  ...
  const confirmarEliminacion = (documento) => {
    confirmDialog({
      message: `¿Está seguro de eliminar el documento "${
        documento.numeroDocumento || "ID: " + documento.id
      }"?`,
  ...
  const tripulanteTemplate = (rowData) => {
    if (!personal || personal.length === 0) {
      return <span className="text-500">Sin personal</span>;
    }
      ...
      <div className="card">
        ...
        >
          ...
          <Column
            ...
            body={(rowData) => (
              <span className="font-semibold">
                {rowData.numeroDocumento || "Sin número"}
              </span>
            )}
          />
          <Column
