// src/pages/PagoCuentaPorCobrar.jsx
...
export default function PagoCuentaPorCobrar() {
  ...
  const savePago = async (data) => {
    try {
      ...
      if (isEdit && selectedPago) {
        await updatePagoCuentaPorCobrar(selectedPago.id, data);
        toast.current?.show({
          severity: "success",
          summary: "Éxito",
        ...
        await createPagoCuentaPorCobrar(data);
        toast.current?.show({
          severity: "success",
          summary: "Éxito",
  ...
  const cuentaPorCobrarBodyTemplate = (rowData) => {
    ...
    return cuenta?.numeroPreFactura || rowData.cuentaPorCobrarId || "-";
  };
  ...
  return (
    <div className="card">
      ...
      <h2>Gestión de Cobros (Pagos de Cuentas por Cobrar)</h2>
      ...
      <Dialog
        ...
        header={isEdit ? "Editar Cobro" : "Registrar Nuevo Cobro"}
        modal
        className="p-fluid"
        onHide={hideDialog}
      >
