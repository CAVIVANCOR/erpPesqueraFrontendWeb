...
import { getAllAccionesPreviasFaena, createAccionesPreviasFaena, updateAccionesPreviasFaena, deleteAccionesPreviasFaena } from "../api/accionesPreviasFaena";
...
export default function AccionesPreviasFaena() {
  ...
  const handleDeleteConfirm = async () => {
    ...
    setLoading(true);
    try {
      await deleteAccionesPreviasFaena(toDelete.id);
      toast.current.show({ severity: "success", summary: "Eliminado", detail: "Acción eliminada correctamente." });
      cargarItems();
    } catch (err) {
  ...
  const handleFormSubmit = async (data) => {
    ...
    try {
      if (editing && editing.id) {
        await updateAccionesPreviasFaena(editing.id, data);
        toast.current.show({ severity: "success", summary: "Actualizado", detail: "Acción actualizada." });
      } else {
        await createAccionesPreviasFaena(data);
        toast.current.show({ severity: "success", summary: "Creado", detail: "Acción creada." });
      }
      setShowDialog(false);
      setEditing(null);
      cargarItems();
  ...
  return (
    <div className="p-fluid">
      <Toast ref={toast} />
      <ConfirmDialog visible={showConfirm} onHide={() => setShowConfirm(false)} message="¿Está seguro que desea eliminar esta acción previa?" header="Confirmar eliminación" icon="pi pi-exclamation-triangle" acceptClassName="p-button-danger" accept={handleDeleteConfirm} reject={() => setShowConfirm(false)} />
      ...
      <Dialog header={editing ? "Editar Acción Previa" : "Nueva Acción Previa"} visible={showDialog} style={{ width: 600 }} onHide={() => setShowDialog(false)} modal>
