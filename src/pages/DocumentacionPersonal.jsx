...
import { getAllDocumentacionPersonal, createDocumentacionPersonal, updateDocumentacionPersonal, deleteDocumentacionPersonal } from "../api/documentacionPersonal";
import { useAuthStore } from "../shared/stores/useAuthStore";

/**
 * Pantalla profesional para gestión de Documentación Personal.
...
export default function DocumentacionPersonal() {
  ...
  const handleDeleteConfirm = async () => {
    ...
    setLoading(true);
    try {
      await deleteDocumentacionPersonal(toDelete.id);
  ...
  const handleFormSubmit = async (data) => {
    ...
    try {
      if (editing && editing.id) {
        await updateDocumentacionPersonal(editing.id, data);
        toast.current.show({ severity: "success", summary: "Actualizado", detail: "Registro actualizado." });
      } else {
        await createDocumentacionPersonal(data);
  ...
  return (
    <div className="p-fluid">
      ...
      <div className="p-d-flex p-jc-between p-ai-center" style={{ marginBottom: 16 }}>
        <h2>Gestión de Documentación Personal</h2>
      ...
      <DataTable value={items} loading={loading} dataKey="id" paginator rows={10} onRowClick={e => handleEdit(e.data)} style={{ cursor: "pointer" }}>
        ...
        <Column field="personalId" header="Personal" />
