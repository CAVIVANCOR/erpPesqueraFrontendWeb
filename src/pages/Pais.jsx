// src/pages/Pais.jsx
// Pantalla CRUD profesional para Pais. Cumple la regla transversal ERP Megui.
...
import { getPaises, crearPais, actualizarPais, eliminarPais } from "../api/pais";
import { useAuthStore } from "../shared/stores/useAuthStore";

/**
 * Pantalla profesional para gestión de Países.
...
export default function Pais() {
  ...
  const handleDeleteConfirm = async () => {
    ...
    setLoading(true);
    try {
      await eliminarPais(toDelete.id);
      toast.current.show({ severity: "success", summary: "Eliminado", detail: "País eliminado correctamente." });
      cargarDatos();
    } catch (err) {
  ...
  const handleFormSubmit = async (data) => {
    ...
    try {
      if (editing && editing.id) {
        await actualizarPais(editing.id, data);
        toast.current.show({ severity: "success", summary: "Actualizado", detail: "País actualizado." });
      } else {
        await crearPais(data);
        toast.current.show({ severity: "success", summary: "Creado", detail: "País creado." });
      }
      setShowDialog(false);
      setEditing(null);
      cargarDatos();
  ...
  return (
    <div className="p-fluid">
      <Toast ref={toast} />
      <ConfirmDialog visible={showConfirm} onHide={() => setShowConfirm(false)} message="¿Está seguro que desea eliminar este país?" header="Confirmar eliminación" icon="pi pi-exclamation-triangle" acceptClassName="p-button-danger" accept={handleDeleteConfirm} reject={() => setShowConfirm(false)} />
      ...
      <DataTable value={items} loading={loading} dataKey="id" paginator rows={10} onRowClick={e => handleEdit(e.data)} style={{ cursor: "pointer" }}>
        ...
        <Column field="gentilicio" header="Gentilicio" />
      ...
      <Dialog header={editing ? "Editar País" : "Nuevo País"} visible={showDialog} style={{ width: 600 }} onHide={() => setShowDialog(false)} modal>
        <PaisForm
          isEdit={!!editing}
