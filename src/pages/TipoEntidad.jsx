...
import { getTiposEntidad, crearTipoEntidad, actualizarTipoEntidad, eliminarTipoEntidad } from "../api/tipoEntidad";
import { useAuthStore } from "../shared/stores/useAuthStore";

/**
 * Pantalla profesional para gestión de Tipos de Entidad.
...
export default function TipoEntidad() {
  ...
  const handleDeleteConfirm = async () => {
    ...
    setLoading(true);
    try {
      await eliminarTipoEntidad(toDelete.id);
      toast.current.show({ severity: "success", summary: "Eliminado", detail: "Tipo de entidad eliminado correctamente." });
      cargarDatos();
    } catch (err) {
  ...
  const handleFormSubmit = async (data) => {
    ...
    try {
      if (editing && editing.id) {
        await actualizarTipoEntidad(editing.id, data);
        toast.current.show({ severity: "success", summary: "Actualizado", detail: "Tipo de entidad actualizado." });
      } else {
        await crearTipoEntidad(data);
        toast.current.show({ severity: "success", summary: "Creado", detail: "Tipo de entidad creado." });
      }
      setShowDialog(false);
      setEditing(null);
      cargarDatos();
  ...
  return (
    <div className="p-fluid">
      <Toast ref={toast} />
      <ConfirmDialog visible={showConfirm} onHide={() => setShowConfirm(false)} message="¿Está seguro que desea eliminar este tipo de entidad?" header="Confirmar eliminación" icon="pi pi-exclamation-triangle" acceptClassName="p-button-danger" accept={handleDeleteConfirm} reject={() => setShowConfirm(false)} />
      ...
      <Dialog header={editing ? "Editar Tipo de Entidad" : "Nuevo Tipo de Entidad"} visible={showDialog} style={{ width: 600 }} onHide={() => setShowDialog(false)} modal>
        <TipoEntidadForm
          isEdit={!!editing}
