...
import { getAllCategoriaCCosto, createCategoriaCCosto, updateCategoriaCCosto, deleteCategoriaCCosto } from "../api/categoriaCCosto";
...
export default function CategoriaCCosto() {
  ...
  const handleDeleteConfirm = async () => {
    ...
    setLoading(true);
    try {
      await deleteCategoriaCCosto(toDelete.id);
      toast.current.show({ severity: "success", summary: "Eliminado", detail: "Categoría eliminada correctamente." });
      cargarItems();
    } catch (err) {
  ...
  const handleFormSubmit = async (data) => {
    ...
    try {
      if (editing && editing.id) {
        await updateCategoriaCCosto(editing.id, data);
        toast.current.show({ severity: "success", summary: "Actualizado", detail: "Categoría actualizada." });
      } else {
        await createCategoriaCCosto(data);
        toast.current.show({ severity: "success", summary: "Creado", detail: "Categoría creada." });
      }
      setShowDialog(false);
      setEditing(null);
      cargarItems();
  ...
  return (
    <div className="p-fluid">
      <Toast ref={toast} />
      <ConfirmDialog visible={showConfirm} onHide={() => setShowConfirm(false)} message="¿Está seguro que desea eliminar esta categoría?" header="Confirmar eliminación" icon="pi pi-exclamation-triangle" acceptClassName="p-button-danger" accept={handleDeleteConfirm} reject={() => setShowConfirm(false)} />
      ...
      <Dialog header={editing ? "Editar Categoría" : "Nueva Categoría"} visible={showDialog} style={{ width: 600 }} onHide={() => setShowDialog(false)} modal>
        <CategoriaCCostoForm
          isEdit={!!editing}
