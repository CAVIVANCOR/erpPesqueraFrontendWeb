...
import { getEspecies, crearEspecie, actualizarEspecie, eliminarEspecie } from '../api/especie';
...
export default function EspeciePage() {
  ...
  const cargarEspecies = async () => {
    ...
    try {
      ...
      toast.current?.show({ severity: 'error', summary: 'Error', detail: 'No se pudo cargar las especies' });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      toast.current?.show({ severity: 'success', summary: 'Eliminado', detail: 'Especie eliminada correctamente' });
      cargarEspecies();
    } catch (err) {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (isEdit && selected) {
        ...
        toast.current?.show({ severity: 'success', summary: 'Actualizado', detail: 'Especie actualizada' });
      } else {
        await crearEspecie(payload);
        toast.current?.show({ severity: 'success', summary: 'Registrada', detail: 'Especie creada' });
      }
      setShowForm(false);
      cargarEspecies();
    } catch (err) {
  ...
  return (
    <div className="p-m-4">
      ...
      <div className="p-d-flex p-jc-between p-ai-center p-mb-3">
        <h2>Especies</h2>
        <Button label="Nueva Especie" icon="pi pi-plus" onClick={onNew} />
      </div>
      <ConfirmDialog
        visible={confirmState.visible}
        onHide={() => setConfirmState({ visible: false, row: null })}
        message={<span style={{ color: '#b71c1c', fontWeight: 600 }}>
          ¿Está seguro que desea <span style={{ color: '#b71c1c' }}>eliminar</span> la especie <b>{confirmState.row ? confirmState.row.nombre : ''}</b>?<br/>
        ...
        style={{ minWidth: 400 }}
      />
      <DataTable value={especies} loading={loading} paginator rows={10} selectionMode="single" selection={selected} onSelectionChange={e => setSelected(e.value)} onRowClick={onRowClick}>
        <Column field="id" header="ID" style={{ width: 80 }} />
        <Column field="nombre" header="Nombre" />
        <Column header="Acciones" body={actionBodyTemplate} style={{ minWidth: 120 }} />
      </DataTable>
      <Dialog header={isEdit ? 'Editar Especie' : 'Nueva Especie'} visible={showForm} style={{ width: 400 }} modal onHide={onCancel}>
        <EspecieForm
          isEdit={isEdit}
