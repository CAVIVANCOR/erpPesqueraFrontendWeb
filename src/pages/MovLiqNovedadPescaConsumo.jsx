// src/pages/MovLiqNovedadPescaConsumo.jsx
// Pantalla CRUD profesional para MovLiqNovedadPescaConsumo. Cumple regla transversal ERP Megui:
...
import { getAllMovLiqNovedadPescaConsumo, deleteMovLiqNovedadPescaConsumo } from '../api/movLiqNovedadPescaConsumo';
import MovLiqNovedadPescaConsumoForm from '../components/movLiqNovedadPescaConsumo/MovLiqNovedadPescaConsumoForm';

/**
 * Componente MovLiqNovedadPescaConsumo
 * Gestión CRUD de movimientos de liquidaciones de novedades de pesca de consumo con patrón profesional ERP Megui
 */
const MovLiqNovedadPescaConsumo = () => {
  ...
  const cargarMovimientos = async () => {
    try {
      setLoading(true);
      const data = await getAllMovLiqNovedadPescaConsumo();
      setMovimientos(data);
    } catch (error) {
      toast.current?.show({
        severity: 'error',
        summary: 'Error',
        detail: 'Error al cargar movimientos de liquidación de novedad'
      });
    } finally {
      setLoading(false);
    }
  ...
  const eliminarMovimiento = async (id) => {
    try {
      await deleteMovLiqNovedadPescaConsumo(id);
      toast.current?.show({
        severity: 'success',
        summary: 'Éxito',
  ...
  const liquidacionTemplate = (rowData) => {
    ...
    const liquidaciones = {
      1: { numero: 'LIQNOV-2024-001', novedad: 'NOV-2024-001', estado: 'APROBADA' },
      2: { numero: 'LIQNOV-2024-002', novedad: 'NOV-2024-002', estado: 'PENDIENTE' },
      3: { numero: 'LIQNOV-2024-003', novedad: 'NOV-2024-003', estado: 'BORRADOR' },
      4: { numero: 'LIQNOV-2024-004', novedad: 'NOV-2024-004', estado: 'APROBADA' }
    };
    
    const liquidacion = liquidaciones[rowData.liqNovedadPescaConsumoId] || { 
      numero: `LIQNOV-${rowData.liqNovedadPescaConsumoId}`, 
      novedad: `NOV-${rowData.liqNovedadPescaConsumoId}`,
      estado: 'BORRADOR'
    };
    ...
    return (
      <div>
        ...
        <div className="text-sm text-gray-600">{liquidacion.novedad}</div>
  ...
  const tipoMovimientoTemplate = (rowData) => {
    const getSeverity = (tipo) => {
      switch (tipo) {
        ...
        case 'COMPENSACION': return 'info';
        case 'PENALIZACION': return 'danger';
        default: return 'secondary';
      }
    };
    ...
    const getIcon = (tipo) => {
      switch (tipo) {
        ...
        case 'COMPENSACION': return 'pi pi-balance-scale';
        case 'PENALIZACION': return 'pi pi-exclamation-triangle';
  ...
  const conceptoTemplate = (rowData) => {
    ...
    const conceptos = {
      'INGRESO': ['Compensación por Novedad', 'Recuperación de Costos', 'Bonificación', 'Otros Ingresos'],
      'EGRESO': ['Pérdida por Novedad', 'Gastos Adicionales', 'Reparaciones', 'Otros Gastos'],
      'AJUSTE': ['Ajuste por Novedad', 'Corrección Contable', 'Regularización'],
      'COMPENSACION': ['Compensación Aseguradora', 'Indemnización', 'Reembolso'],
      'PENALIZACION': ['Penalización por Incumplimiento', 'Multa', 'Sanción']
    };
  ...
  const montoTemplate = (rowData) => {
    const isIngreso = rowData.tipoMovimiento === 'INGRESO' || rowData.tipoMovimiento === 'COMPENSACION';
  ...
  const centroCostoTemplate = (rowData) => {
    ...
    const centrosCosto = {
      ...
      3: { codigo: 'CC003', nombre: 'Seguros', tipo: 'Financiero' },
  ...
  const prioridadTemplate = (rowData) => {
    ...
    return (
      <Tag 
        value={getLabel(rowData.prioridad)} 
        severity={getSeverity(rowData.prioridad)} 
      />
    );
  };
  ...
  return (
    <div className="mov-liq-novedad-pesca-consumo-container">
      <Toast ref={toast} />
      <ConfirmDialog />
      
      <div className="card">
        <div className="flex justify-content-between align-items-center mb-4">
          <h2>Movimientos de Liquidación de Novedad - Pesca Consumo</h2>
          <Button
        ...
        <DataTable
          ...
          emptyMessage="No se encontraron movimientos de liquidación de novedad"
          scrollable
          scrollHeight="600px"
        >
          ...
          />
          <Column 
            field="liqNovedadPescaConsumoId" 
            header="Liquidación" 
      ...
      <Dialog
        ...
        style={{ width: '900px' }}
        header={isEditing ? 'Editar Movimiento de Liquidación de Novedad' : 'Nuevo Movimiento de Liquidación de Novedad'}
        modal
        onHide={cerrarDialogo}
      >
        <MovLiqNovedadPescaConsumoForm
...
export default MovLiqNovedadPescaConsumo;
