// src/pages/DetalleOrdenCompra.jsx
// Pantalla CRUD profesional para DetalleOrdenCompra. Cumple regla transversal ERP Megui:
 ...
 * Gestión CRUD de detalles de órdenes de compra con patrón profesional ERP Megui
 */
const DetalleOrdenCompra = () => {
  ...
  const cargarDetalles = async () => {
    try {
      setLoading(true);
      const data = await getAllDetalleOrdenCompra();
      setDetalles(data);
    } catch (error) {
      toast.current?.show({
        severity: 'error',
        summary: 'Error',
        detail: 'Error al cargar detalles de órdenes de compra'
      });
    } finally {
      setLoading(false);
    }
  ...
  const eliminarDetalle = async (id) => {
    try {
      await deleteDetalleOrdenCompra(id);
      toast.current?.show({
        severity: 'success',
        summary: 'Éxito',
  ...
  const cantidadTemplate = (rowData) => {
    return formatearNumero(rowData.cantidadPedida, 2);
  };

  const cantidadRecibidaTemplate = (rowData) => {
    return formatearNumero(rowData.cantidadRecibida, 2);
  };
  ...
  const subtotalTemplate = (rowData) => {
    const subtotal = (rowData.cantidadPedida || 0) * (rowData.precioUnitario || 0);
  ...
  return (
    <div className="detalle-orden-compra-container">
      <Toast ref={toast} />
      <ConfirmDialog />
      
      <div className="card">
        <div className="flex justify-content-between align-items-center mb-4">
          <h2>Detalles de Órdenes de Compra</h2>
          <Button
            label="Nuevo Detalle"
            icon="pi pi-plus"
        ...
        >
          ...
          <Column field="ordenCompraId" header="Orden ID" sortable style={{ width: '100px' }} />
          ...
          />
          <Column 
            field="fechaEntregaRequerida" 
            header="F. Entrega Req." 
            body={(rowData) => formatearFecha(rowData.fechaEntregaRequerida)}
            sortable 
            style={{ width: '130px' }}
          />
          <Column 
            field="fechaEntregaReal" 
            header="F. Entrega Real" 
            body={(rowData) => formatearFecha(rowData.fechaEntregaReal)}
            sortable 
            style={{ width: '130px' }}
          />
          <Column 
