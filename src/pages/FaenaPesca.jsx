// src/pages/FaenaPesca.jsx
// Pantalla CRUD profesional para FaenaPesca. Cumple la regla transversal ERP Megui.
...
import FaenaPescaForm from "../components/faenaPesca/FaenaPescaForm";
import { getFaenasPesca, crearFaenaPesca, actualizarFaenaPesca, eliminarFaenaPesca } from "../api/faenaPesca";
import { useAuthStore } from "../shared/stores/useAuthStore";

/**
 * Pantalla profesional para gestión de Faenas de Pesca.
 ...
 */
export default function FaenaPesca() {
  ...
  const handleDeleteConfirm = async () => {
    ...
    setLoading(true);
    try {
      await eliminarFaenaPesca(toDelete.id);
  ...
  return (
    <div className="p-fluid">
      ...
      <div className="p-d-flex p-jc-between p-ai-center" style={{ marginBottom: 16 }}>
        <h2>Gestión de Faenas de Pesca</h2>
      ...
      <DataTable value={items} loading={loading} dataKey="id" paginator rows={10} onRowClick={e => handleEdit(e.data)} style={{ cursor: "pointer" }}>
        ...
        <Column field="temporadaId" header="Temporada" />
        <Column field="bahiaId" header="Bahía" />
        <Column field="motoristaId" header="Motorista" />
        ...
        <Column field="urlInformeFaena" header="URL Informe" />
        <Column body={actionBody} header="Acciones" style={{ width: 130, textAlign: "center" }} />
      </DataTable>
      <Dialog header={editing ? "Editar Faena de Pesca" : "Nueva Faena de Pesca"} visible={showDialog} style={{ width: 800 }} onHide={() => setShowDialog(false)} modal>
        <FaenaPescaForm
          isEdit={!!editing}
