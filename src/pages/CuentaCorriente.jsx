...
import { getAllCuentaCorriente, createCuentaCorriente, updateCuentaCorriente, deleteCuentaCorriente } from "../api/cuentaCorriente";
...
export default function CuentaCorriente() {
  ...
  const handleDeleteConfirm = async () => {
    ...
    setLoading(true);
    try {
      await deleteCuentaCorriente(toDelete.id);
  ...
  return (
    <div className="p-fluid">
      ...
      <DataTable value={items} loading={loading} dataKey="id" paginator rows={10} onRowClick={e => handleEdit(e.data)} style={{ cursor: "pointer" }}>
        ...
        <Column field="bancoId" header="Banco" />
        <Column field="numeroCuenta" header="Número de Cuenta" />
        <Column field="tipoCuentaCorrienteId" header="Tipo Cuenta" />
        <Column field="monedaId" header="Moneda" />
        <Column field="descripcion" header="Descripción" />
        <Column field="activa" header="Activa" body={rowData => rowData.activa ? "Sí" : "No"} />
        <Column body={actionBody} header="Acciones" style={{ width: 130, textAlign: "center" }} />
      </DataTable>
      <Dialog header={editing ? "Editar Cuenta Corriente" : "Nueva Cuenta Corriente"} visible={showDialog} style={{ width: 700 }} onHide={() => setShowDialog(false)} modal>
        <CuentaCorrienteForm
          isEdit={!!editing}
