// src/pages/TipoMantenimiento.jsx
// Pantalla profesional de gestión de tipos de mantenimiento para el ERP Megui.
...
} from "../api/tipoMantenimiento";
import TipoMantenimientoForm from "../components/tipoMantenimiento/TipoMantenimientoForm";
import { getResponsiveFontSize } from "../utils/utils";

/**
 * Página de gestión de tipos de mantenimiento.
 ...
 */
export default function TipoMantenimiento({ ruta }) {
  ...
  async function cargarItems() {
    ...
    try {
      ...
      mostrarToast("error", "Error", "No se pudieron cargar los tipos de mantenimiento");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function onSubmitForm(data) {
    ...
    try {
      ...
      if (modoEdicion && itemEdit) {
        await actualizarTipoMantenimiento(itemEdit.id, payload);
        mostrarToast(
          "success",
          "Tipo actualizado",
          "El tipo de mantenimiento fue actualizado correctamente."
        );
      } else {
        await crearTipoMantenimiento(payload);
        mostrarToast(
          "success",
          "Tipo creado",
          "El tipo de mantenimiento fue registrado correctamente."
        );
      }
      setMostrarDialogo(false);
      cargarItems();
    } catch (err) {
      mostrarToast("error", "Error", "No se pudo guardar el tipo de mantenimiento.");
    } finally {
  ...
  const handleConfirmDelete = async () => {
    ...
    setLoading(true);
    try {
      await eliminarTipoMantenimiento(item.id);
      mostrarToast(
        "success",
        "Tipo eliminado",
      ...
      mostrarToast("error", "Error", "No se pudo eliminar el tipo de mantenimiento.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div style={{ maxWidth: 1200, margin: "0 auto", padding: "2rem 0" }}>
      ...
      <ConfirmDialog
        ...
        message={
          <span style={{ color: "#b71c1c", fontWeight: 600 }}>
            ...
            <span style={{ color: "#b71c1c" }}>eliminar</span> el tipo de mantenimiento{" "}
      ...
      <DataTable
        ...
        currentPageReportTemplate="Mostrando {first} a {last} de {totalRecords} tipos de mantenimiento"
        size="small"
        showGridlines
        stripedRows
