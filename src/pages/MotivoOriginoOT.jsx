...
// Pantalla CRUD profesional para MotivoOriginoOT. Cumple la regla transversal ERP Megui.
...
import { getAllMotivoOriginoOT, createMotivoOriginoOT, updateMotivoOriginoOT, deleteMotivoOriginoOT } from "../api/motivoOriginoOT";
import { useAuthStore } from "../shared/stores/useAuthStore";

/**
 * Pantalla profesional para gestión de Motivos que originan OT.
 ...
 */
export default function MotivoOriginoOT() {
  ...
  const cargarItems = async () => {
    setLoading(true);
    try {
      const data = await getAllMotivoOriginoOT();
      setItems(data);
    } catch (err) {
  ...
  const handleDeleteConfirm = async () => {
    ...
    setLoading(true);
    try {
      await deleteMotivoOriginoOT(toDelete.id);
  ...
  return (
    <div className="p-fluid">
      ...
      <div className="p-d-flex p-jc-between p-ai-center" style={{ marginBottom: 16 }}>
        <h2>Gestión de Motivos que originan OT</h2>
      ...
      <Dialog header={editing ? "Editar Motivo" : "Nuevo Motivo"} visible={showDialog} style={{ width: 500 }} onHide={() => setShowDialog(false)} modal>
        <MotivoOriginoOTForm
          isEdit={!!editing}
