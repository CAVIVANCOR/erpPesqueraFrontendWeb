...
// Pantalla CRUD profesional para Departamento. Cumple la regla transversal ERP Megui.
...
import { getDepartamentos, crearDepartamento, actualizarDepartamento, eliminarDepartamento } from "../api/departamento";
import { getPaises } from "../api/pais";
import { useAuthStore } from "../shared/stores/useAuthStore";

/**
 * Pantalla profesional para gestión de Departamentos.
...
export default function Departamento() {
  ...
  const handleDeleteConfirm = async () => {
    ...
    setLoading(true);
    try {
      await eliminarDepartamento(toDelete.id);
      toast.current.show({ severity: "success", summary: "Eliminado", detail: "Departamento eliminado correctamente." });
      cargarDatos();
    } catch (err) {
  ...
  const handleFormSubmit = async (data) => {
    ...
    try {
      if (editing && editing.id) {
        await actualizarDepartamento(editing.id, data);
        toast.current.show({ severity: "success", summary: "Actualizado", detail: "Departamento actualizado." });
      } else {
        await crearDepartamento(data);
        toast.current.show({ severity: "success", summary: "Creado", detail: "Departamento creado." });
      }
      setShowDialog(false);
      setEditing(null);
      cargarDatos();
  ...
  return (
    <div className="p-fluid">
      <Toast ref={toast} />
      <ConfirmDialog visible={showConfirm} onHide={() => setShowConfirm(false)} message="¿Está seguro que desea eliminar este departamento?" header="Confirmar eliminación" icon="pi pi-exclamation-triangle" acceptClassName="p-button-danger" accept={handleDeleteConfirm} reject={() => setShowConfirm(false)} />
      ...
      <Dialog header={editing ? "Editar Departamento" : "Nuevo Departamento"} visible={showDialog} style={{ width: 600 }} onHide={() => setShowDialog(false)} modal>
        <DepartamentoForm
          isEdit={!!editing}
